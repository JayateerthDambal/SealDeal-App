  ...
  useEffect(() => {
    ...
    const unsubscribeDeal = onSnapshot(dealRef, (doc) => {
      if (doc.exists()) {
        ...
        if (prevStatusRef.current !== '4_Analyzed' && dealData.status === '4_Analyzed') {
           setActiveTab('analysis');
        }
        
        setDeal(dealData);
        prevStatusRef.current = dealData.status;
    ...
    <div className="space-y-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <div className="flex gap-2">
                ...
                >
                  {isAnalyzing || deal.status === '2_Processing' ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Zap className="mr-2 h-4 w-4" />}
                  Run Analysis
                </Button>
            </div>
        </div>
